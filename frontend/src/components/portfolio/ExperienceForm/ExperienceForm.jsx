...
const ExperienceForm = () => {
  ...
  const addItem = () => {
    updateSection("experience", [...items, { id: Date.now(), role: '', company: '', period: '', description: '' }]);
  };

  const removeItem = (id) => {
    updateSection("experience", items.filter(item => item.id !== id));
  };

  const handleChange = (id, field, value) => {
    updateSection("experience", items.map(item => item.id === id ? { ...item, [field]: value } : item));
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <input 
                  type="text" 
                  value={item.period}
                  onChange={(e) => handleChange(item.id, 'period', e.target.value)}
